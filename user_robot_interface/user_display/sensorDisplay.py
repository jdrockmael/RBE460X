...
def updateLeftSensor(msg):
    global leftSensor
    leftSensor = msg.data
def updateRightSensor(msg):
    global rightSensor
    rightSensor= msg.data
def callback(data):
    global speed, turning, maxSpeed, maxTurning, oldTime, mathInterval, firstVar, secondVar, thirdVar, queastionIndex, leftSensor, rightSensor
    global offsetCenter, widthSpeedBar, widthBar, radiusCircle
    ...
    splitWidth=int(w/70)
    widthBar=20
    leftSensorLevel=leftSensor/12
    leftSideHeight=int(leftSensorLevel*12)
    rightSensorLevel=rightSensor/12
    rightSideHeight=int(rightSensorLevel*12)
    cv2.rectangle(current_frame, (w-splitWidth,h-(splitHeight*3)), (w-splitWidth-widthBar,splitHeight), green, 2)
    cv2.rectangle(current_frame, (splitWidth,h-(splitHeight*3)), (splitWidth+widthBar,splitHeight), green, 2)

    #indicator bar
    cv2.rectangle(current_frame, (w-splitWidth,h-(splitHeight*3)), (w-splitWidth-widthBar,h-(splitHeight*3)-(splitHeight*rightSideHeight)), red, -1)
    cv2.rectangle(current_frame, (splitWidth,h-(splitHeight*3)), (splitWidth+widthBar,h-(splitHeight*3)-(splitHeight*leftSideHeight)), red, -1)
...
def receive_message():
    global mathInterval, offsetCenter, widthSpeedBar, widthBar, radiusCircle, maxSpeed, maxTurning
    ...
    rospy.Subscriber('leftRumbleLevel', Int16, updateLeftSensor)
    rospy.Subscriber('rightRumbleLevel', Int16, updateRightSensor)
